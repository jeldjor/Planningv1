...
window.GJ_AUTH={sb,profile:null,isAdmin:false,workspaceUserId:null,impersonating:false};
window.GJ_WORKSPACE_ID=()=>window.GJ_AUTH?.workspaceUserId||window.GJ_AUTH?.profile?.id||null;
...
async function profileFor(user){const email=(user.email||'').trim().toLowerCase();let {data,error}=await sb.from('profiles').select('*').eq('id',user.id).maybeSingle();if(error)throw error;const m=user.user_metadata||{};const forcedAdmin=email==='georgio_jejanan@hotmail.com';if(!data){const row={id:user.id,email:user.email,first_name:m.first_name||'',last_name:m.last_name||'',full_name:m.full_name||user.email,role:forcedAdmin?'admin':'user',is_active:true};const up=await sb.from('profiles').upsert(row).select().single();if(up.error)throw up.error;data=up.data}else if(forcedAdmin&&data.role!=='admin'){const fix=await sb.from('profiles').update({role:'admin'}).eq('id',user.id).select().single();if(!fix.error&&fix.data)data=fix.data}return data}
async function openApp(session){const p=await profileFor(session.user);if(p.is_active===false){await sb.auth.signOut();throw new Error('Dit account is uitgeschakeld.')}GJ_AUTH.profile=p;GJ_AUTH.isAdmin=String(p.role||'').toLowerCase()==='admin'||String(session.user.email||'').toLowerCase()==='georgio_jejanan@hotmail.com';const imp=GJ_AUTH.isAdmin?sessionStorage.getItem('gj_impersonate_user_v102'):null;GJ_AUTH.workspaceUserId=imp||p.id;GJ_AUTH.impersonating=!!imp;document.body.classList.toggle('gj-admin',GJ_AUTH.isAdmin);document.getElementById('gjAuthGate').style.display='none';document.querySelectorAll('.gjUserBar').forEach(x=>x.remove());const st=await sb.rpc('get_tomtom_status');window.GJ_TOMTOM_ENABLED=!!(st.data===true||st.data?.enabled===true);window.dispatchEvent(new CustomEvent('gj-auth-ready',{detail:{profile:p}}))}
