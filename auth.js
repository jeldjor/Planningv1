...
async function openApp(session){
 ...
 const name=p.full_name||[p.first_name,p.last_name].filter(Boolean).join(' ')||p.email||'Gebruiker';
 const roleLabel=p.role==='admin'?'Beheerder':'Gebruiker';
 window.GJ_CURRENT_USER={name,role:roleLabel,profile:p};
 document.querySelectorAll('.gjUserBar').forEach(el=>el.remove());
 document.body.insertAdjacentHTML('beforeend',`<div class="gjUserBar"><span>${name} · ${roleLabel}</span><button id="gjLogout">Uitloggen</button></div>`);
 document.getElementById('gjLogout').onclick=async()=>{
   try{localStorage.removeItem('bp_user_name');localStorage.removeItem('bp_user_email')}catch(e){}
